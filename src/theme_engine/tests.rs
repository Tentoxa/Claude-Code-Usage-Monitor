...
fn starter_theme_round_trips_and_validates() {
    ...
    // five providers over two windows in two modes, one Fable scoped window,
    // plus a credit overlay on the weekly row of the two credit providers.
    assert_eq!(segments, vec![10; 5 * 2 * 2 + 1 * 2 + 2 * 2]);
...
#[test]
fn fable_scope_and_missing_windows_are_available_to_the_classic_theme() {
    use crate::models::{UsageData, UsageSection};
    use std::time::{Duration, SystemTime};

    let reset = SystemTime::now() + Duration::from_secs(3_600);
    let usage = AppUsageData::from_iter([
        (
            ProviderId::Claude,
            UsageData {
    ...
    let runtime = ThemeRuntime::new(true, true, false).with_poll_state(true, false);
    let context = DataContext::from_usage_with_runtime(Some(&usage), &Canvas::default(), runtime);

    assert_eq!(context.get("claude.fable.available"), Some(1.0));
    assert_eq!(context.get("claude.fable.percentage"), Some(61.0));
    assert_eq!(context.get("codex.session.available"), Some(0.0));
    assert_eq!(context.get("codex.weekly.available"), Some(1.0));
    assert_eq!(context.get("windows.session.available"), Some(1.0));
    assert_eq!(context.get("windows.weekly.available"), Some(1.0));
    assert_eq!(
        format_template("{claude.fable:usage_line}", &context),
        "61% · 1h"
    );
