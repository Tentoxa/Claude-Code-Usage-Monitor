...
impl DataContext {
    ...
    pub fn from_usage_with_runtime(
        ...
        let window_is_available = |section: &crate::models::UsageSection| {
            section.resets_at.is_some() || section.percentage != 0.0
        };
        let (session_available, weekly_available) = if runtime.poll_ok && data.is_some() {
            let session = data.is_some_and(|data| {
                runtime.providers.iter().any(|provider| {
                    data.get(provider)
                        .is_some_and(|usage| window_is_available(&usage.session))
                })
            });
            let weekly = data.is_some_and(|data| {
                runtime.providers.iter().any(|provider| {
                    data.get(provider).is_some_and(|usage| {
                        window_is_available(&usage.weekly)
                            || usage.fable.is_some()
                            || usage.credits.is_some()
                            || usage.monthly.is_some()
                    })
                })
            });
            (session, weekly)
        } else {
            // Preserve placeholders until a successful response containing
            // provider data confirms that a window is genuinely absent.
            (true, true)
        };
        context.insert("windows.session.available", session_available as u8 as f64);
        context.insert("windows.weekly.available", weekly_available as u8 as f64);
        context
    }
    ...
    fn insert_provider(
        ...
        self.insert(
            &format!("{name}.session.available"),
            usage.is_some_and(|usage| window_is_available(&usage.session)) as u8 as f64,
        );
        self.insert(
        ...
        self.insert(
            &format!("{name}.weekly.available"),
            usage.is_some_and(|usage| window_is_available(&usage.weekly)) as u8 as f64,
        );
        let fable = usage.and_then(|usage| usage.fable.as_ref());
        let fable_percentage = fable.map(|section| section.percentage).unwrap_or_default();
