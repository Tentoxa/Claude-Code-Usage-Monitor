...
struct UsageResponse {
    ...
    /// Model-scoped allowances, including the optional Fable weekly limit.
...
/// Locate the optional weekly Fable allowance in Anthropic's scoped limits.
/// Accounts without that limit keep `fable` absent, so themes hide it.
fn extract_fable_section(limits: Option<&[UsageLimit]>) -> Option<crate::models::UsageSection> {
    ...
    Some(crate::models::UsageSection {
        percentage: limit.percent.unwrap_or_default(),
        ...
        let data = usage_from_json(
            r#"{
                ...
                "limits": [{
                    "kind": "weekly_scoped",
                    "percent": 61,
                    "resets_at": "2026-08-08T04:00:00.465217+00:00",
                    "scope": {"model": {"display_name": "Fable"}}
                }]
            }"#,
        );

        let fable = data.fable.expect("Fable limit should be present");
        assert_eq!(fable.percentage, 61.0);
        assert!(fable.resets_at.is_some());
    }

    #[test]
    fn omits_fable_when_an_account_has_no_scoped_limit() {
        let data = usage_from_json(
            r#"{
                "five_hour": {"utilization": 9.0},
